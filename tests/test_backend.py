...
def test_check_token(test_client, init_db):
    monkey = create_users_movies_and_tags()[0]
    headers = {"Authorization": "Bearer " + monkey.token}
    res = test_client.post("/api/checktoken", headers=headers)
    assert res.status_code == 200
    assert res.json["user"] == "monkey"
    assert res.json["email"] == None


def test_get_movies_no_user(test_client, init_db):
    monkey, bella, hazel, movie_1, movie_2, movie_3, action, comedy, documentary = (
        create_users_movies_and_tags()
    )
    monkey.saves.append(movie_2)
...
def test_get_movies_user(test_client, init_db):
    monkey, bella, hazel, movie_1, movie_2, movie_3, action, comedy, documentary = (
        create_users_movies_and_tags()
    )
    monkey.saves.append(movie_2)
...
def test_add_user(test_client, init_db):
    ...
    assert "laura" not in [u.username for u in User.query.all()]
    ...
    assert "laura" in [u.username for u in User.query.all()]
...
def test_update_account(test_client, init_db):
    ...
    assert monkey.username == "monkeycat"
    assert res.json["user"] == "monkeycat"
    # email is same
    assert monkey.email == None
    assert res.json["email"] == None
    # password is same
    ...
    j_data = json.dumps(
        {"newUserName": "belladog", "newEmail": "bella@dog.com", "newPassword": None}
    )
    res = test_client.post(
        "/api/updateaccount",
        headers=headers,
    ...
    assert res.status_code == 200
    # username should have changed
    assert bella.username == "belladog"
    assert res.json["user"] == "belladog"
    # email should have changed
    assert bella.email == "bella@dog.com"
    assert res.json["email"] == "bella@dog.com"
    # password is same
    ...
    assert res.status_code == 200
    # username should have changed
    assert hazel.username == "hazeldog"
    assert res.json["user"] == "hazeldog"
    # email should have changed
    assert hazel.email == "hazel@dog.com"
    assert res.json["email"] == "hazel@dog.com"
...
def test_save_movie(test_client, init_db):
    monkey, bella, hazel, movie_1, movie_2, movie_3, action, comedy, documentary = (
        create_users_movies_and_tags()
    )
    headers = {"Authorization": "Bearer " + monkey.token}
...
def test_unsave_movie(test_client, init_db):
    monkey, bella, hazel, movie_1, movie_2, movie_3, action, comedy, documentary = (
        create_users_movies_and_tags()
    )
    movie_1.savers.append(monkey)
